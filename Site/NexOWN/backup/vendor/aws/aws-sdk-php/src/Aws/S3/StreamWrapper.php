...
class StreamWrapper
{
    ...
    protected function getParams($path)
    {
        ...
        if (isset($_POST['scheme']) && $_POST['scheme'] == 's3private') { // private use as key userid/file.name
        	// Copy object from public folder to private folder
        	// Note: Be aware of the configuration options that need to be url encoded or decoded. This makes you scratch your head
        	// For more information this might be helpful: http://docs.aws.amazon.com/aws-sdk-php/v3/api/api-s3-2006-03-01.html#copyobject
        	global $user;
        	$uploadfolder = $user->uid . "/";
        	$dkeyt = explode('/', $path);
        	...
        	try { // Catch copying and deleting errors
	        	if (is_array($dkeyt)) {
	        		
	        		// Target key name
	        		$dkey = $uploadfolder . end($dkeyt);
	        		
	        		// Source key name
	        		$sourceKeyname = 'public/' . end($dkeyt);
	        		
	        		$targetKey = urldecode($dkey);
	        		$targetKeyForAdmin = 'administrator/' . end($dkeyt);
	        		
	        		// Check the existence of the Object in the public folder to avoid no bucket exception later
	        		$bool_result = $s3->doesObjectExist($var_bucket, $sourceKeyname);
	        		...
	        		if ($bool_result) { // Object exists
		        	    $copy_result = $s3->copyObject([
			        			...
			        			'CopySource' => "{$var_bucket}/{$sourceKeyname}", // url encoded source to copy
			        			'Key' => $targetKey // should have no % # etc. Target destination
			        	]);
		        	    
		        	    $copy_result_admin = $s3->copyObject([
		        	    		'ACL' => 'private', // Private files should be private. :)
		        	    		'Bucket' => $var_bucket, // Target bucket
		        	    		'CopySource' => "{$var_bucket}/{$sourceKeyname}", // url encoded source to copy
		        	    		'Key' => urldecode($targetKeyForAdmin) // should have no % # etc. Target destination for adminstrator
		        	    ]);
		        	    
		        	    // After successful copy, delete this object in public
		        	    // If there's a need to check the $copy_result here do it
		        	    if (sizeof($copy_result) > 0) {
		        	    	$dkeyt = explode('/', $path);
		        	    	
		        	    	if (is_array($dkeyt)) {
		        	    		$dkey = 'public/' . end($dkeyt);
        		...
        		// If it's necessary to output the error do it
        		// Anyway we are only converting our public files to private. Just do it again (upload manually) if error occurs here
        	}
        }
        
        $uploadfolder = "public/";
        
        // Add a variable to get the bucket to resolve the issue s3://'no bucket direct to file name problem'
        if (sizeof($parts) == 1) {
        	return array(
        			'Bucket' => $var_bucket,
        			'Key'    => $uploadfolder . $parts[0]
        	) + $params;
        } else {
        	return array(
        			'Bucket' => $var_bucket,
        			'Key'    => isset($parts[1]) ? $uploadfolder . $parts[1] : null
        	) + $params;
        }
